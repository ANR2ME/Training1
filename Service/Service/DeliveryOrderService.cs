...
namespace Service.Service
{
    public class DeliveryOrderService : IDeliveryOrderService
    {
        ...
        public DeliveryOrder CreateObject(DeliveryOrder deliveryOrder, ISalesOrderService _salesOrderService, IContactService _contactService)
        {
            deliveryOrder.Errors = new Dictionary<String, String>();
            return (deliveryOrder = _validator.ValidCreateObject(deliveryOrder, _salesOrderService, _contactService) ? _repository.CreateObject(deliveryOrder) : deliveryOrder);
        }

        public DeliveryOrder UpdateObject(DeliveryOrder deliveryOrder, ISalesOrderService _salesOrderService, IContactService _contactService)
        {
            return (deliveryOrder = _validator.ValidUpdateObject(deliveryOrder, _salesOrderService, _contactService) ? _repository.UpdateObject(deliveryOrder) : deliveryOrder);
        }
